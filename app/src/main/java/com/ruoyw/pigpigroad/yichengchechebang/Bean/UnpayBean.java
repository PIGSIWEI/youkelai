package com.ruoyw.pigpigroad.yichengchechebang.Bean;

/**
 * Created by PIGROAD on 2018/11/1
 * Email:920015363@qq.com
 */
public class UnpayBean {

    public String gun_number;

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String id;
    public String pay_money;
    public String gun_kind;
    public String orderid;
    public String pay_time;
    public String order_time;
    public String name;
    public String merchant_oil_price;
    public String use_oil_price;
    public String oil_money;
    public String coupon_money;
    public String oil_lit;
    public String oil_name;
    public String gun_id;

    public String getGun_number() {
        return gun_number;
    }

    public void setGun_number(String gun_number) {
        this.gun_number = gun_number;
    }

    public String getPay_money() {
        return pay_money;
    }

    public void setPay_money(String pay_money) {
        this.pay_money = pay_money;
    }

    public String getGun_kind() {
        return gun_kind;
    }

    public void setGun_kind(String gun_kind) {
        this.gun_kind = gun_kind;
    }

    public String getOrderid() {
        return orderid;
    }

    public void setOrderid(String orderid) {
        this.orderid = orderid;
    }

    public String getPay_time() {
        return pay_time;
    }

    public void setPay_time(String pay_time) {
        this.pay_time = pay_time;
    }

    public String getOrder_time() {
        return order_time;
    }

    public void setOrder_time(String order_time) {
        this.order_time = order_time;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getMerchant_oil_price() {
        return merchant_oil_price;
    }

    public void setMerchant_oil_price(String merchant_oil_price) {
        this.merchant_oil_price = merchant_oil_price;
    }

    public String getUse_oil_price() {
        return use_oil_price;
    }

    public void setUse_oil_price(String use_oil_price) {
        this.use_oil_price = use_oil_price;
    }

    public String getOil_money() {
        return oil_money;
    }

    public void setOil_money(String oil_money) {
        this.oil_money = oil_money;
    }

    public String getCoupon_money() {
        return coupon_money;
    }

    public void setCoupon_money(String coupon_money) {
        this.coupon_money = coupon_money;
    }

    public String getOil_lit() {
        return oil_lit;
    }

    public void setOil_lit(String oil_lit) {
        this.oil_lit = oil_lit;
    }

    public String getOil_name() {
        return oil_name;
    }

    public void setOil_name(String oil_name) {
        this.oil_name = oil_name;
    }

    public String getGun_id() {
        return gun_id;
    }

    public void setGun_id(String gun_id) {
        this.gun_id = gun_id;
    }
}
